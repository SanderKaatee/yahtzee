<script>
  ...
  import { room, gameState, players, spectators, isPlaying, isWaiting, isFinished } from '$lib/stores/game.js';
  ...
  const canRoll = $derived(isMyTurn && ($gameState?.rollsLeft || 0) > 0 && !$gameState?.isRolling);
  const canHold = $derived(isMyTurn && ($gameState?.rollsLeft || 0) < 3 && ($gameState?.rollsLeft || 0) > 0);
  ...
  function handleRoll() {
    ...
    socket.emit('roll-dice', { roomId, playerId: $player.id });
  }
  
  function handleToggleHold(index) {
    const socket = getSocket();
    if (!socket || !canHold) return;
    socket.emit('toggle-hold', { roomId, playerId: $player.id, dieIndex: index });
  }
  ...
  function handleKick(targetPlayerId) {
    const socket = getSocket();
    if (!socket || !isHost) return;
    socket.emit('kick-player', { roomId, playerId: $player.id, targetPlayerId });
  }
...
<div class="min-h-screen p-4 pb-safe">
  ...
  <div class="flex items-center justify-between mb-4">
    ...
    <div class="text-center">
      ...
      {#if $isPlaying}
        <span class="text-sm text-slate-400">Round {$gameState?.turnNumber || 1} of 13</span>
      {:else if $isWaiting}
    ...
    <div class="max-w-md mx-auto space-y-4">
      ...
      {:else if isSpectator}
        <div class="card text-center">
          ...
          <p class="text-sm text-slate-500">Waiting for host to start the game...</p>
        </div>
      {:else}
        <div class="card text-center">
  ...
  {:else if $isPlaying}
    ...
    <div class="max-w-4xl mx-auto">
      ...
      <div class="text-center mb-4">
        {#if isMyTurn}
          <span class="text-lg font-semibold text-yahtzee-green">🎯 Your turn!</span>
        {:else}
          <span class="text-slate-400">
            Waiting for <span class="font-semibold text-white">{currentPlayer?.name}</span>...
          </span>
        {/if}
      </div>
  ...
  {:else if $isFinished}
    ...
    <div class="max-w-md mx-auto space-y-4">
      <div class="card text-center">
        ...
        {#if $gameState?.winner}
          <p class="text-lg">
            <span class="font-semibold text-yahtzee-green">{$gameState.winner.name}</span> wins!
          </p>
      ...
      <div class="card">
        ...
        <div class="space-y-2">
          {#each [...$players].sort((a, b) => calculateTotalScore(b.scorecard) - calculateTotalScore(a.scorecard)) as p, i}
            <div class="flex justify-between items-center p-2 rounded {i === 0 ? 'bg-yahtzee-green/20' : ''}">
              <span>
                {#if i === 0}🥇{:else if i === 1}🥈{:else if i === 2}🥉{:else}{i + 1}.{/if}
                {p.name}
              </span>
              <span class="font-mono font-bold">{calculateTotalScore(p.scorecard)}</span>
            </div>
          {/each}
        </div>
      </div>
